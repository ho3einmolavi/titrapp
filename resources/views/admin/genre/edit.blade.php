...
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <a href="{{ route('genre.index') }}" class="btn btn-success"><i
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            <form class="form-horizontal" action="{{ route('genre.update',$genre->id) }}" method="post" enctype="multipart/form-data">
                {{ csrf_field() }}
                {{ method_field('PATCH') }}
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="type" class="form-control select2" name="category_id"  style="width: 100%;">
                                        ...
                                        @foreach(\App\Category::orderBy('name')->get() as $category)
                                            <option value="{{$category->id}}" {{$genre->category_id == $category->id ? 'selected' : ''}}>{{$category->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
